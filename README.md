...
worker 端另有兩種**預期外失敗**（訊息進 DLQ，驗證哨兵分流的完整路徑）：

| 情境 | 機率/chunk | 哨兵處置 |
|------|--------|------|
| 暫時性故障（embedding 5xx / OOM） | 0.4% | 自動重放 → 第二次成功 ✅ |
| 黏性失敗（特定輸入觸發的未知 bug，hash 決定） | 0.03% | 重放 2 次仍失敗 → **留 DLQ 等人工**（`C2DLQNotEmpty`） |

`ERROR_MULTIPLIER=0` 可關閉 uploader 隨機注入；`TRANSIENT_FAIL_PROB=0 STICKY_FAIL_PROB=0`
可關閉 worker 端失敗（演示前清場用）。
